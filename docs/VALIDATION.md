...
The [STM32F446 common160 diagnostic](../hardware/common160/2026-09-11/stm32f446_diagnostic_01.json) passed all twelve workloads with exact call counts and final DONE through USART3/PC10. It reported nominal CPU 160000000 Hz, APB1/APB2 40000000/80000000 Hz, Scale 1 ready with OverDrive disabled, five Flash wait states with prefetch and instruction/data caches enabled, full FPU access and TIM2 prescaler 7999. The matching silent release BIN was [programmed afterward](../hardware/common160/2026-09-11/stm32f446_measurement_programming_01.json) through ST-LINK mass storage, with completed host transfer and no `FAIL.TXT` after five seconds. Common160 is the STM32's latest recorded installation. This programming observation does not independently validate flash contents or the silent image's execution.

Common160 functional diagnostics and subsequent silent-image programming are now recorded for all three boards. All three still require new common160 PPK2 pilots and ten accepted captures each; see [COMMON_CLOCK_160.md](COMMON_CLOCK_160.md). The ten accepted ESP32 captures at 240 MHz remain selected for the maximum-clock campaign with their original v3 provenance; no repeat 240 MHz acquisition is required.
...
[CURRENT_FIRMWARE.json](../CURRENT_FIRMWARE.json) records maximum-clock image hashes, compilation, historical programming and validation separately. Maximum-clock measurement archives are under `firmware/targets/<target>/build_verified/max_clock_measurement`. The manifest's `single_gpio_measurement` profile key remains for collector compatibility and points to that location. The earlier Pico 200 MHz and STM32 180 MHz PASS and programming records remain valid historical observations; all three boards now have common160 as their latest recorded installation. See the [hardware report](HARDWARE_VALIDATION.md) for observed device results.
...
The [STM32 functional diagnostic](../hardware/2026-09-11/stm32f446_pc10_diagnostic_01.json) passed all twelve workloads with their exact call counts and final DONE at the nominal 180 MHz setting. Its separate USART3/PC10 transport used target CN7 pin 1 to detached ST-LINK CN3 pin 1 (RX), with common ground, because open SB63 blocked PA2 TX at CN9 pin 2. It reported the expected nominal CPU/bus clocks, FPU access, Flash and power-control registers, and TIM2 prescaler. The diagnostic transport change preserved the clock setup, kernels, inputs and call counts. Afterward, the original published serial-disabled measurement BIN was [reinstalled through ST-LINK mass storage](../hardware/2026-09-11/stm32f446_measurement_programming_02.json) without `FAIL.TXT`; that 180 MHz installation was subsequently replaced by the common160 image recorded above. No flash-readback attestation was obtained; execution of the silent measurement image and PPK2 validation remain pending. Exact image hashes and the separate diagnostic/programming evidence are recorded in the maximum-clock manifest and [hardware report](HARDWARE_VALIDATION.md).
