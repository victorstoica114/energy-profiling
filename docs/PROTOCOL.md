...
The completed campaigns are **`energy-profiling-v4-max-clock`** and **`energy-profiling-v5-common160`**, both experiment schema 2. [experiment.json](../config/experiment.json) defines the parameters and the [input manifest](../data/manifest.json) identifies the exact bytes. The [maximum-clock selection](../ROW_Data/source_campaign.json) retains ten ESP32 v3 captures and adds twenty RP2040/STM32 v4 captures; the [common160 selection](../ROW_Data/common160/source_campaign.json) contains thirty new v5 captures. Each source keeps its original metadata and expected-image identity.

## Select the experiment

The physical protocol below applies to both experiments. **max_clock remains the default**. For the new equal-frequency comparison use `--clock-profile common160`; this selects `config/experiment.common160.json`, `profiles/common160/CURRENT_FIRMWARE.json` and `captures_common160/`. All three CPUs then use nominal 160 MHz. The completed common160 selection contains ten new accepted cold boots per board; no earlier capture or idle baseline is reused. Follow the same independent-capture procedure for any new campaign. See the [common160 guide](COMMON_CLOCK_160.md) for the supported operating points and campaign template.

## Power and wiring

Use PPK2 in Source Meter mode at a nominal 3300 mV. Connect VOUT to the measured supply domain: ESP32/Pico 3V3, or the MCU VDD side of open JP6 on Nucleo. Connect GND to DUT GND and LOGIC VCC to that measured supply. Connect **only the measurement output to PPK2 D0**:
...
ESP32/Pico board voltage regulators are physically removed. On NUCLEO-F446RE the LDO remains fitted, JP6 is open, and PPK2 feeds the MCU VDD side of JP6; the measurement covers the circuitry connected to MCU VDD and excludes the isolated LDO and circuitry remaining on its board-side rail. Completed campaigns retain this operator-confirmed correction separately from original acquisition records. Internal MCU regulators remain part of the device. In max_clock, RP2040 selects 1.15 V for 200 MHz, STM32F446 uses Scale 1 and OverDrive at nominal 180 MHz, and ESP32 uses 240 MHz. In common160, RP2040 retains 1.15 V, STM32 uses Scale 1 with OverDrive disabled, and ESP-IDF applies its normal 160 MHz digital bias. These internal settings are documented in the common160 guide. Verify the current image and recorded configuration before acquisition; changing the PPK2 supply is not how the RP2040 core-voltage setting is applied.
...
`tools/capture_ppk2.py` provides an automated alternative for cold-boot runs through `ppk2-api==0.9.2`. It starts sampling with VOUT off, applies the configured 3.3 V setpoint, observes D0, and stops only after twelve falling edges and at least 3 s of trailing LOW. Every run retains the exact four-byte transport stream, a generic CSV, structural-analysis outputs, calibration/device metadata and hashes. Its uniform CSV timebase is reconstructed from sample indices because the API stream has no independent per-sample timestamps. The collector checks the PPK2 rolling 6-bit sample counter and host elapsed time; loss of an exact multiple of 64 samples can evade the rolling-counter check, so this is stronger evidence but not an absolute loss-free proof. Do not relabel its `.raw4` file as a native Nordic `.ppk2`; for the strongest provenance, also preserve a pilot captured with the official application.

The automated collector refuses to enable VOUT until `--confirm-wiring` is supplied. Run `python tools/capture_ppk2.py --list-devices` first, then acquire one pilot before requesting the minimum ten independent captures. It always requests DUT power OFF on normal exit, rejection, interruption and timeout.

The default first-edge watchdog is 30 s after VOUT is enabled. This is deliberately longer than the nominal startup pause but prevents an absent/miswired D0 signal from leaving a pilot powered until the full capture timeout. After an abruptly terminated external terminal, use `python tools/capture_ppk2.py --power-off` and confirm the instrument state before touching the fixture.
...
Use one complete suite per file. The analyzer requires exactly twelve complete HIGH pulses, a valid LOW prefix of at least the nominal 5 s and eleven LOW gaps of at least the nominal 1 s. ESP32 and RP2040 use a **1% tolerance** for those MCU pause lower bounds. NUCLEO-F446RE uses a **2% tolerance** because its control timer is derived from the internal HSI16 RC oscillator. This structural allowance must be checked in each new setup pilot; it is not an external clock calibration. It is reported in every analysis and does not alter RUN-window integration. No upper bound is used because initialization/preparation/verification can lengthen LOW. At 100 kS/s, the minimum counts are 495000/99000 for ESP32 and RP2040, 490000/98000 for F446, and **300000 trailing LOW samples** for every board. The baseline uses only the final 200000 samples before the first pulse; entire gaps are not labeled active idle.
