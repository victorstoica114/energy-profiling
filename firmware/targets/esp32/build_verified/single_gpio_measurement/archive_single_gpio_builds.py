...
def main() -> None:
    ...
    parser.add_argument("--refresh-new-archive", action="store_true")
    ...
    for target in args.target or ("esp32", "rp2040", "stm32f446"):
        ...
        if (archive / "verification.json").exists():
            prior = json.loads((archive / "verification.json").read_text())
            if not args.refresh_new_archive or prior.get("profile") != "single_gpio_measurement" or prior.get("hardware_flashed") or prior.get("campaign") != manifest.get("experiment_id"):
